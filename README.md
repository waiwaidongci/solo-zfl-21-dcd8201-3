...
复测的 `testedAt` 必须是可解析的时间字符串（如 `2026-09-01T00:00:00.000Z`），空值、非字符串或不可解析时间均返回 `400` 且不写入；字段缺失时仍自动使用当前时间。

新增钟表的 `code` 必须唯一：编号去除前后空白后做大小写不敏感比较，重复编号返回 `409` 且不写入；编号保存时会去除前后空白并保留原大小写。
